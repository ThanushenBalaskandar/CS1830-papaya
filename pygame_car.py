...
def game_loop():
    ...
    while not gameExit:
        ...
        load_image(papaya_startx, papaya_starty, 'images/papaya.png')

        papaya_starty += papaya_speed
        ...
        if papaya_starty > display_height:
            papaya_starty = 0 - 50
            papaya_startx = random.randrange(170, display_width-50-150)
            thing_speed += 1/20
        ...
        # Papaya collision
        if y < (papaya_starty + 50) and y + car_height >= papaya_starty:
            if x >= papaya_startx and x <= (papaya_startx + 50) or x + car_width >= papaya_startx \
